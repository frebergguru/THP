...
if ($action=="LAdded") {
	...
	$query_string = "INSERT INTO `boxes` ( `id` , `site` , `catid` , `position` , `headline` , `uheadline` , `link` , `text` ) VALUES ('', '$id2', '$id2', 'l', '$editheadline', '$edituheadline', '$editlink', '$edittext')";
...
//else if $action is like "LAddedI" then
    ...
    {	//if a error has ocured then
		...
		//insert the path to the image to the database
            $query_string = "INSERT INTO `boxes` ( `id` , `site` , `catid` , `position` , `headline` , `image` ) VALUES ('', '$id2', '$id2', 'l', '$editheadline', '$editimage')";
            mysql_query("$query_string") or mysqldie("Kan ikke skrive til $database.boxes");
            }
        }
...
};//Query the database table boxes where catid is $id2 and position is l order by id ascending
$result=mysql_query("SELECT * FROM boxes WHERE catid='$id2' AND position='l' ORDER BY id asc") or mysqldie("Kan ikke lese fra $database.boxes");
...
<tbody>
<tr>
<td class="under"><strong><a href="?site='.$site.'&amp;id2='.$id2.'&amp;action=';if (!empty($image)) {print 'LEditI';}else{print 'LEdit';};print '&amp;id='.$id.'">Rediger</a> || <a href="?site='.$site.'&amp;id2='.$id2.'&amp;action=';if(!empty($image)){print 'LDeleteI';} else {print 'LDelete';};print'&amp;id='.$id.'">Slett</a></strong></td>
</tr>
<tr>
<td class="over2">
