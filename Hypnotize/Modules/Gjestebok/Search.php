...
*/
$search = (string) $_POST['search']; //Find out what the user is searcing for
//if $search not is empty then
if (!empty($search)) {
//query the table guestbook where ?? is like $search
$result=mysql_query("SELECT * FROM guestbook WHERE message LIKE '%".mysql_real_escape_string($search)."%' OR name LIKE '%".mysql_real_escape_string($search)."%' OR ip LIKE '%".mysql_real_escape_string($search)."%' ORDER BY id") or mysqldie ("Kan ikke lese fra $database.guestbook");
...
<tr class="under">
<td valign="top">
S&oslash;ket etter "'.stripslashes(chchar($search)).'" ga '.$tr.' resultater!
</td>
</tr>
</tbody>
</table>
...
<strong>Fra:</strong> <a href="mailto:'.$mail.'">'.$name.'</a> <strong>Dato:</strong> '.$date.'
</font></td>
</tr>
<tr>
...
<font size="1">
<a href="'.$homepage.'" class="under2"><strong>Hjemmeside</strong></a>
</font>
</td>
</tr>
</tbody>
